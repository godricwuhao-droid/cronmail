  ...
  <div class="page-container">
    <div class="page-header">
      <div class="flex items-center gap-base">
        <el-button @click="router.back()"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>返回</el-button>
        <h1 class="page-title">{{ isEdit ? '编辑合同' : '新建合同' }}</h1>
      </div>
    </div>
    <div class="content-card">
      <el-form :model="form" label-width="100px" class="contract-form">
        <div class="form-section"><div class="form-section__title">基础信息</div>
          <div class="form-grid">
            <el-form-item label="合同编号" required><el-input v-model="form.contract_no" placeholder="例如：CT-2026-001" /></el-form-item>
            <el-form-item label="合同标题" required><el-input v-model="form.title" placeholder="例如：2026年卫星数据协议" /></el-form-item>
        ...
        <div class="form-section"><div class="form-section__title">时间配置</div>
          <div class="form-grid">
        ...
        <div class="form-section"><div class="form-section__title">其他信息</div>
          <div class="form-grid form-grid--full"><el-form-item label="描述"><el-input v-model="form.description" type="textarea" :rows="3" placeholder="请输入合同描述信息" /></el-form-item></div>
        </div>
...
onMounted(async () => {
  try { customers.value = await getCustomers({ page_size: 200 }) } catch { /* ignore */ }
  if (isEdit) { try { Object.assign(form, await getContract(id!)) } catch { /* ignore */ } }
})
const handleSave = async () => {
  loading.value = true
  try { if (isEdit) await updateContract(id!, form); else { const res: any = await createContract(form); router.push(`/contracts/satellite/${res.id}`) }; ElMessage.success('保存成功'); router.push('/contracts/satellite') } catch { /* ignore */ } finally { loading.value = false }
}
</script>
