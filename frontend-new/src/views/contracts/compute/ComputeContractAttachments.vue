  ...
  <div class="page-container">
    <div class="page-header">
      <div class="flex items-center gap-base">
        <el-button @click="router.push(`/contracts/compute/${contractId}`)">
...
const loadData = async () => {
  loading.value = true
  try {
    const res: any = await getAttachments({ related_type: 'compute_contract', related_id: contractId, page_size: 100 })
...
const handleUpload = async (e: Event) => {
  ...
  formData.append('related_type', 'compute_contract')
  formData.append('related_id', contractId)
  try { await uploadAttachment(formData); ElMessage.success('上传成功'); loadData() } catch { /* ignore */ }
...
.empty-state__description { font-size: var(--font-size-base); color: var(--color-text-tertiary); }
</style>