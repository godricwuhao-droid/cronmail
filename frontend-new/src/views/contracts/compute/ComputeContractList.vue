  ...
  <div class="page-container">
    ...
    <div class="page-toolbar">
      ...
      <div class="page-toolbar-right">
        ...
        <el-button type="primary" :icon="Plus" @click="router.push('/contracts/compute/create')">新建合同</el-button>
      </div>
    </div>
    ...
    <div class="content-card" style="padding: 0;">
      ...
      >
        ...
        <el-table-column v-if="visibleColumns.some(c => c.key === 'name')" prop="name" label="合同名称" min-width="220" show-overflow-tooltip>
          <template #default="{ row }">
            <span class="contract-title" @click="router.push(`/contracts/compute/${row.id}`)">{{ row.name }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="220" fixed="right" align="center">
          <template #default="{ row }">
            <el-button link type="primary" @click="router.push(`/contracts/compute/${row.id}`)">详情</el-button>
            <el-button link type="primary" @click="router.push(`/contracts/compute/${row.id}/edit`)">编辑</el-button>
      ...
      <el-empty v-if="!loading && list.length === 0" description="暂无合同数据" style="padding: 60px 20px;">
        <el-button type="primary" @click="router.push('/contracts/compute/create')">新建合同</el-button>
      </el-empty>
...
const {
  ...
  toggleColumn,
  resetColumns,
} = useColumnCustomization(defaultColumns, 'contract-columns')
...
const handleMore = (command: string, row: any) => {
  if (command === 'renew') {
    router.push(`/contracts/compute/${row.id}/renew`)
  } else if (command === 'attachments') {
    router.push(`/contracts/compute/${row.id}/attachments`)
