<template>
  <div class="page-container">
    <div class="page-header"><h1 class="page-title">蜂云时代项目</h1></div>
      ...
      <el-table :data="list" v-loading="loading" stripe>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" @click="router.push(`/projects/fengyun/${row.id}`)">详情</el-button>
            <el-button link type="primary" @click="router.push(`/projects/fengyun/${row.id}/edit`)">编辑</el-button>
            <el-button link type="primary" @click="router.push(`/projects/fengyun/${row.id}/attachments`)">附件</el-button>
...
const total = ref(0)
onMounted(() => loadData())
const loadData = async () => { loading.value = true; try { const res: any = await getProjectContracts('fengyun', { page: page.value, page_size: pageSize.value, keyword: keyword.value }); list.value = res.items || res; total.value = res.total || 0 } catch { /* ignore */ } finally { loading.value = false } }
const handleDelete = async (id: string) => { try { await deleteProjectContract('fengyun', id); ElMessage.success('删除成功'); loadData() } catch { /* ignore */ } }
</script>
