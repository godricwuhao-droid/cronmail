    ...
    <div class="content-card">
      <el-form :model="form" label-width="100px" class="contract-form">
        <div class="form-section"><div class="form-section__title">基础信息</div><div class="form-grid">
          <el-form-item label="项目名称" required><el-input v-model="form.name" placeholder="例如：蜂云时代一期项目" /></el-form-item>
...
onMounted(async () => { try { customers.value = await getCustomers({ page_size: 200 }) } catch { /* ignore */ } if (isEdit) { try { Object.assign(form, await getProjectContract('fengyun', id!)) } catch { /* ignore */ } } })
const handleSave = async () => { loading.value = true; try { if (isEdit) await updateProjectContract('fengyun', id!, form); else { const res: any = await createProjectContract('fengyun', form); router.push(`/projects/fengyun/${res.id}`) }; ElMessage.success('保存成功'); router.push('/projects/fengyun') } catch { /* ignore */ } finally { loading.value = false } }
</script>
